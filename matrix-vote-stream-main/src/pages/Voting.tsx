...
const Voting = () => {
  ...
  useEffect(() => {
    ...
    if (now < electionWithDates.startDate) {
      setError(`Voting has not started yet. Voting will begin on ${electionWithDates.startDate.toLocaleDateString()} at ${electionWithDates.startDate.toLocaleTimeString()}`);
    } else if (now > electionWithDates.endDate) {
      setError(`Voting period has ended. Voting closed on ${electionWithDates.endDate.toLocaleDateString()} at ${electionWithDates.endDate.toLocaleTimeString()}`);
    }
    ...
    // Also check if this voter has voted in any other election (one person one vote)
    const voterRegistrations = JSON.parse(localStorage.getItem('voterRegistrations') || '[]');
    const currentVoterRegistration = voterRegistrations.find((reg: any) => reg.voterIdHash === currentVoterId);

    if (currentVoterRegistration) {
      const hasVotedInOtherElection = votes.some((vote: any) =>
        vote.voterId === currentVoterId && vote.electionId !== electionId
      );

      if (hasVotedInOtherElection) {
        setError('You have already voted in another election. One person can only vote once across all elections.');
        return;
      }
    }
  ...
  const submitVote = async () => {
    ...
    const now = new Date();
    if (now > currentElection.endDate) {
      setError(`Voting period has ended. Voting closed on ${currentElection.endDate.toLocaleDateString()} at ${currentElection.endDate.toLocaleTimeString()}`);
      return;
    }
    ...
    try {
      ...
      const voterName = localStorage.getItem('currentVoterName') || 'Unknown Voter';
